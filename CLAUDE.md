...
5. **`cli/`** — Tell/Ask CLI with REPL mode (`--onto` flag enables ontology mode):
   ...
   - `pynmms repl` — interactive session with tell/ask/show/save/load; a `_Session` per command, sharing `tell._process_tell_statement` and `ask._ask_one` (no second implementation of either)
...
833 tests across 40 test files:

**Propositional core (393 tests, 15 files):**
...
**Ontology extension (230 tests, 7 files):**
...
**RDF extension (210 tests, 18 files):**
...
- `test_rdf_repl.py` — the `pynmms rdf repl` position client through scripted stdin: every command in the help table, tell/deny/withdraw and their errors, coherent with the entry and rescue named, challenges, propose writing nothing, position, entitlement and defend, commit with the holder's attribution, save/load, unknown commands, quit and EOF
