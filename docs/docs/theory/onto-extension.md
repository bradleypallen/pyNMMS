...
**Non-transitivity.** In OWL, `SubClassOf(A, B)` and `SubClassOf(B, C)` entail `SubClassOf(A, C)`. In NMMS_Onto, `subClassOf(A, B)` and `subClassOf(B, C)` do not jointly entail `{A(x)} |~ {C(x)}`. The calculus lacks [Mixed-Cut], so individually valid inferences do not compose. If the chain should hold, it must be registered explicitly as `subClassOf(A, C)`. (The RDF layer's `RegimeBase` is different: an entailment regime is defined by closure, so chains compose there, as Section 8.4 explains.) This is by design: some subclass chains should compose (Man -> Mortal -> Physical) and others should not, depending on the domain. The representationalist reading, which treats subClassOf as a subset relation, makes non-transitivity look like a bug. The inferentialist reading, which treats each schema as an independent defeasible commitment, makes it intelligible: undertaking a commitment to A-entails-B and a commitment to B-entails-C does not automatically constitute a commitment to A-entails-C.

**Defeasibility.** In OWL, if `Man SubClassOf Mortal`, then adding any further assertions about Socrates cannot defeat the inference from `Man(socrates)` to `Mortal(socrates)`. In NMMS_Onto with the default `exact` policy, `{Man(socrates), Immortal(socrates)} |~ {Mortal(socrates)}` is not derivable -- the additional premise defeats the inference because the antecedent no longer exactly matches the schema pattern. Since v0.7 a schema can instead be `guarded` by named defeater concepts (`subClassOf(Bird, Flies) unless Penguin`), so that irrelevant additions such as `Tall(tweety)` leave the inference intact and only `Penguin(tweety)` defeats it, or `monotone`, which is the OWL reading. The representationalist reading, which treats class membership as extensional and monotonic, provides no resources for understanding why adding information could defeat an inference. The inferentialist reading makes it immediate: a schema records a commitment undertaken under a range of subjunctive robustness (Hlobil & Brandom 2025, Ch. 5), and the policy says how wide that range is. `exact` is the narrowest, `monotone` the widest, and `guarded` names the specific additions that fall outside it.
...
### 8.4 From NMMS_Onto to NMMS over RDF

Allen (2026) gives RDF itself an implication-space semantics, and `pynmms.rdf` implements it (see [RDF Semantics](rdf-semantics.md)). In that setting the atomic bearers are ground triples, and NMMS_Onto's vocabulary is the `rdf:type` fragment of the encoding: `C(x)` is `(x rdf:type C)` and `R(x, y)` is `(x R y)`. Schema commitments become schema triples (`rdfs:subClassOf`, `rdfs:range`, `rdfs:domain`, `rdfs:subPropertyOf`, `owl:disjointWith`, `owl:propertyDisjointWith`), which an entailment regime reads through its rules; `jointCommitment` becomes a rule. `onto_to_graph`, `onto_to_rules`, and `consequences_to_triples` perform the translation.
...
2. **Exact match is one point on a range of robustness, not the only one**: Through v0.6 every schema generated axioms with singleton antecedent and singleton consequent (pair antecedent and empty consequent for incompatibilities) matched exactly, so that *any* additional premise defeated the inference. Allen (2026) observes, in the remark following `def:fitness`, that such a base "corresponds to no regime" and that its failure of monotonicity is "by omission rather than by defeat". Since v0.7 every base entry and schema carries an explicit robustness policy fixing a fragment of its range of subjunctive robustness (Hlobil & Brandom 2025, Ch. 5): `exact` (the v0.6 behaviour), `monotone` (the regime-base reading, in which additions never defeat), or `guarded` by a set of defeater concepts (additions never defeat except a named defeater on an individual of the matched consequent). `exact` remains the default so existing bases keep their meaning; `guarded` is the recommended policy for material inferential commitments, because it distinguishes relevant defeat (`Penguin` defeats `Bird |~ Flies`) from the arbitrary defeat of exact matching. Containment is unaffected by the policy, so the metatheory of Section 4 is unchanged.
...
1. **Schema interaction principles**: Should there be systematic rules for how schemas interact? For instance, should `subClassOf(C, D)` and `domain(R, C)` jointly entail anything about `R` and `D`? In classical ontology languages they do, via monotonic closure, and for the regime bases of `pynmms.rdf` the question is answered the same way: an entailment regime is defined by closure, so its rules interact freely (rdfs2 and rdfs9 chain, scm-dom1 composes domain with subclass, and so on), and Theorem \ref{thm:closure} (Closure regimes) of Allen (2026) guarantees that the closure is exactly the regime's own entailment. For NMMS_Onto and for `guarded` entries the answer is now the regime-relative material base `B_{R,I}` of [RDF Semantics](rdf-semantics.md), Section 8: a material entry's antecedent, defeaters, and consequent are all read through the regime's closure, so schemas and material entries interact through the regime, while material entries still do not chain with one another. Whether material entries should ever compose, and under what guard, is still open.
