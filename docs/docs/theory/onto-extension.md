...
1. **Schema interaction principles**: Should there be systematic rules for how schemas interact? For instance, should `subClassOf(C, D)` and `domain(R, C)` jointly entail anything about `R` and `D`? In classical ontology languages they do, via monotonic closure, and for the regime bases of `pynmms.rdf` the question is answered the same way: an entailment regime is defined by closure, so its rules interact freely (rdfs2 and rdfs9 chain, scm-dom1 composes domain with subclass, and so on), and Theorem 35 of Allen (2026) guarantees that the closure is exactly the regime's own entailment. For NMMS_Onto and for `guarded` entries the question stays open: closure is the wrong operation for defeasible commitments, because composing two guarded inferences would need a guard for the composite, and nothing in the two guards determines it. Whether there are principled forms of controlled interaction that preserve the nonmonotonic character while recovering some useful entailments is still to be worked out.
